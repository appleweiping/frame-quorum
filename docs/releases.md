...
Versioned Frame Quorum source distributions and wheels are published on the repository's
...
```bash
gh release download v0.2.0 --repo appleweiping/frame-quorum --dir frame-quorum-v0.2.0
cd frame-quorum-v0.2.0
sha256sum --check SHA256SUMS
gh attestation verify ./*.whl --repo appleweiping/frame-quorum
gh attestation verify ./*.tar.gz --repo appleweiping/frame-quorum
```
