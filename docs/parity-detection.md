...
| [CSV `load-scenes`](https://github.com/Breakthrough/PySceneDetect/blob/24953b0bf76af17c450bc143d330eea48fc5e276/scenedetect/_cli/controller.py#L160-L201): bypass detection and feed loaded cuts to later outputs | Strict bounded one-based `Start Frame` CSV import feeds exact-CFR video-only FCPXML and a separate VFR-aware stills/HTML review bundle, without detector statistics; see [FCPXML](native-load-fcpxml.md) and [overview](native-load-overview.md) contracts | `Start Timecode`/alternate columns, VFR editor mapping, reference sorting/postprocessing and general chained outputs remain open |
